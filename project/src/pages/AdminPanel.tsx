...
export interface Product {
  ...
  image_url: string;
  gallery: string[];
  colors: ProductColor[] | string[] | null;
...
const normalizeProductColors = (colors: Product['colors']): ProductColor[] => {
  ...
  return colors
    .map((color) => {
      ...
      return {
        ...
        gallery: Array.isArray(color.gallery)
          ? color.gallery.filter((url): url is string => typeof url === 'string' && url.trim().length > 0)
          : Array.isArray((color as ProductColor & { images?: string[] }).images)
            ? (color as ProductColor & { images?: string[] }).images!.filter((url): url is string => typeof url === 'string' && url.trim().length > 0)
            : [],
        thumbnail: typeof color.thumbnail === 'string' ? color.thumbnail : '',
      };
    })
    .filter((color) => color.name || color.hex || color.thumbnail || color.gallery.length > 0);
};

const createEmptyProductColor = (): ProductColor => ({
  hex: '',
  name: '',
  gallery: [],
  thumbnail: '',
...
export default function AdminPanel({ initialTab = 'analytics' }: { initialTab?: AdminTab }) {
  ...
  // Admin entry scroll reset
  // Always start AdminPanel at the top when entering this route.
  // Synchronize Lenis and the native document scroller so the previous page's
  // scroll position cannot leak into the admin page.
  useEffect(() => {
    if (globalLenis) {
      globalLenis.resize();
  ...
  const getItemImage = (item: OrderItem) => {
    ...
    if (itemColor && Array.isArray(prod?.colors)) {
      const matchedColor = prod.colors.find((color) => {
        const colorName = typeof color === 'string' ? color : color?.name;
        return colorName?.trim().toLowerCase() === itemColor.trim().toLowerCase();
      });
      const thumbnail = typeof matchedColor === 'string' ? '' : matchedColor?.thumbnail;
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messages.filter((message) => message.email.toLowerCase().includes(query));
  ...
  const updateProductColor = (index: number, field: keyof ProductColor, value: string | string[]) => {
    setProductForm((prev) => ({
      ...prev,
  ...
  const addProductColor = () => {
    setProductForm((prev) => ({
      ...prev,
      colors: [...prev.colors, createEmptyProductColor()],
    }));
  };
  ...
  const setProductColorGallery = (index: number, value: string) => {
    updateProductColor(
      index,
      'gallery',
  ...
  const handleSaveProduct = () => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        const { created_at: _createdAt, ...editableProductFields } = productForm;
        const categoryArray = productForm.category
          .split(',')
        ...
        const payload = {
          ...
          colors: productForm.colors.map((color) => ({
            ...
            gallery: color.gallery.filter((url) => url.trim().length > 0).map((url) => url.trim()),
            thumbnail: color.thumbnail.trim(),
          })).filter((color) => color.name || color.hex || color.thumbnail || color.gallery.length > 0),
        };
    ...
    <div className="bg-white dark:bg-neutral-950 min-h-screen text-neutral-900 dark:text-neutral-100 transition-colors duration-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'products' && (
          <>
            <div className="mb-6">
              <h2 className="text-xl font-bold mb-3">public.products</h2>
              {!isAddingProduct && !editingProductId && (
                <div className="grid grid-cols-2 gap-3 w-full">
                  ...
                  <button
                    ...
                    className="w-full min-w-0 h-11 flex items-center justify-center gap-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-950 font-semibold px-4 rounded-lg text-sm whitespace-nowrap hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors"
                  >
                    <Plus className="w-4 h-4 shrink-0" /> Add Product
                  </button>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {editingProductId && (
                    <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-xs uppercase text-neutral-500 dark:text-neutral-400 mb-1">created_at</label>
                        <input
                          type="text"
                          readOnly
                          value={productForm.created_at || 'N/A'}
                          className="w-full bg-neutral-100 dark:bg-neutral-950/50 border border-neutral-200 dark:border-neutral-800/80 rounded-lg px-3 py-2 text-base sm:text-sm font-mono text-neutral-500 dark:text-neutral-400 cursor-not-allowed focus:outline-none"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="block text-xs uppercase text-neutral-500 dark:text-neutral-400 mb-1">rating</label>
                    <input
                      type="number"
                      min="0"
                      max="5"
                      ...
                      onChange={(e) => setProductForm({ ...productForm, rating: e.target.value === '' ? 0 : Number(e.target.value) })}
                      className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-lg px-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700"
                    />
                  </div>
                  ...
                  <div className="md:col-span-2 lg:col-span-3">
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <div>
                        <label className="block text-xs uppercase text-neutral-500 dark:text-neutral-400">colors (JSONB)</label>
                        <p className="text-[11px] text-neutral-400 dark:text-neutral-500 mt-1">Each color uses: hex, name, gallery[], thumbnail</p>
                      </div>
                      <button
                        type="button"
                        onClick={addProductColor}
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-neutral-900 dark:bg-white text-white dark:text-neutral-950 text-xs font-semibold hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors"
                      >
                    ...
                    <div className="space-y-4">
                      {productForm.colors.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-neutral-300 dark:border-neutral-700 p-5 text-center text-sm text-neutral-500 dark:text-neutral-400">
                          No colors added. Click “Add Color” to create the first color.
                        </div>
                      ) : (
                        productForm.colors.map((color, index) => (
                          <div key={`product-color-${index}`} className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50/70 dark:bg-neutral-950/60 p-4">
                            <div className="flex items-center justify-between gap-3 mb-4">
                              <div className="flex items-center gap-2">
                                <span
                                  className="w-7 h-7 rounded-full border border-neutral-300 dark:border-neutral-700 shrink-0"
                                  style={{ backgroundColor: color.hex || 'transparent' }}
                                />
                                <span className="text-sm font-semibold text-neutral-900 dark:text-white">Color {index + 1}</span>
                              </div>
                              <button
                                type="button"
                                onClick={() => removeProductColor(index)}
                                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
                              >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <input
                                  ...
                                  placeholder="red"
                                  className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700"
                                />
                              </div>

                              <div>
                                <label className="block text-xs uppercase text-neutral-500 dark:text-neutral-400 mb-1">hex</label>
                                <div className="flex gap-2">
                                  <input
                                    type="color"
                                    value={/^#[0-9A-Fa-f]{6}$/.test(color.hex) ? color.hex : '#000000'}
                                    onChange={(e) => updateProductColor(index, 'hex', e.target.value.toUpperCase())}
                                    className="w-11 h-10 p-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg cursor-pointer shrink-0"
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => updateProductColor(index, 'hex', e.target.value.toUpperCase())}
                                    placeholder="#FF0F00"
                                    className="flex-1 min-w-0 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700"
                                  />
                                </div>
                              </div>

                              <div className="md:col-span-2">
                                <label className="block text-xs uppercase text-neutral-500 dark:text-neutral-400 mb-1">thumbnail</label>
                                <input
                                  type="text"
                                  value={color.thumbnail}
                                  onChange={(e) => updateProductColor(index, 'thumbnail', e.target.value)}
                                  placeholder="https://.../0.webp"
                                  className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700"
                                />
                              </div>
                              ...
                              <div className="md:col-span-2">
                                ...
                                <textarea
                                  rows={3}
                                  value={color.gallery.join(', ')}
                                  onChange={(e) => setProductColorGallery(index, e.target.value)}
                                  placeholder="https://.../1.webp, https://.../2.webp, https://.../3.webp"
                                  className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700 resize-y"
                                />
                              </div>
                            </div>
                          </div>
                  ...
                  <table className="w-full min-w-[900px] text-left text-sm">
                    <thead className="bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400">
                      <tr>
                        ...
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[220px]" onClick={() => handleProductSort('name')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Name / Slug <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[120px]" onClick={() => handleProductSort('price')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Price <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[190px]" onClick={() => handleProductSort('category')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Category <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[100px]" onClick={() => handleProductSort('stock_quantity')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Stock <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium text-right whitespace-nowrap min-w-[140px]">
                          Actions
                        </th>
                      </tr>
                    </thead>
                      ...
                      {sortedProducts.length === 0 ? (
                        ...
                        <tr key={product.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/50 transition-colors">
                          ...
                          <td className="p-4 min-w-[220px]">
                            <div className="font-semibold text-neutral-900 dark:text-white whitespace-nowrap">{product.name}</div>
                            <div className="text-xs text-neutral-500 dark:text-neutral-400 font-mono whitespace-nowrap">{product.slug}</div>
                          </td>
                          <td className="p-4 font-mono whitespace-nowrap">
                            {product.price !== null ? `$${product.price.toFixed(2)}` : 'NULL'}
                          </td>
                          <td className="p-4 min-w-[190px] whitespace-nowrap">
                            <div className="flex items-center gap-1.5 flex-nowrap whitespace-nowrap">
                              {(Array.isArray(product.category) ? product.category : [product.category]).filter(Boolean).map((category, index) => (
                                <span
                                  key={`${product.id}-category-${index}`}
                                  className="inline-flex items-center whitespace-nowrap px-2.5 py-1 rounded-full text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300"
                                >
                                  {category}
                                </span>
                              ))}
        ...
        {activeTab === 'digital_files' && (
          <>
            <div className="mb-6">
              <h2 className="text-xl font-bold mb-3">public.digital_files</h2>
              {!isAddingFile && !editingFileId && (
                <div className="grid grid-cols-2 gap-3 w-full">
                  ...
                  <button
                    ...
                    className="w-full min-w-0 h-11 flex items-center justify-center gap-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-950 font-semibold px-4 rounded-lg text-sm whitespace-nowrap hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors"
                  >
                    <Plus className="w-4 h-4 shrink-0" /> Add Digital File
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg pl-9 pr-3 py-2 text-base sm:text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:border-neutral-400 dark:focus:border-neutral-700"
                />
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[1040px] text-left text-sm">
                    <thead className="bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400">
                      <tr>
                        <th className="p-4 w-14 whitespace-nowrap"></th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[150px]" onClick={() => handleOrderSort('id')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Order ID <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[230px]" onClick={() => handleOrderSort('email')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Customer <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[140px]" onClick={() => handleOrderSort('status')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Status <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[130px]" onClick={() => handleOrderSort('total')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Total <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[140px]" onClick={() => handleOrderSort('created_at')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Date <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium text-right whitespace-nowrap min-w-[140px]">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                      {sortedOrders.length === 0 ? (
                        <tr>
                          <td colSpan={7} className="p-10 text-center text-sm text-neutral-500 dark:text-neutral-400">
                            {orderSearch.trim() ? 'No orders match your search.' : 'No orders found.'}
                          </td>
                        </tr>
                          ...
                          <React.Fragment key={order.id}>
                            <tr
                              className={`transition-colors ${
                                isExpanded
                                  ? 'bg-neutral-50 dark:bg-neutral-950/60'
                            ...
                            >
                              <td className="p-4 w-14">
                                <button
                                  ...
                                  className="w-9 h-9 flex items-center justify-center text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors"
                                >
                                  <ChevronDown className={`w-5 h-5 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
                                </button>
                              </td>

                              <td className="p-4 font-mono font-semibold text-neutral-900 dark:text-white whitespace-nowrap">
                                #{order.id.slice(0, 8)}
                              </td>

                              <td className="p-4 min-w-[230px]">
                                <div className="font-semibold text-neutral-900 dark:text-white whitespace-nowrap">{order.full_name || 'Guest'}</div>
                                <div className="text-xs text-neutral-500 dark:text-neutral-400">{order.email || 'No email'}</div>
                              </td>
  ...
  <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${
    ...
    {order.status}
                              ...
                              <td className="p-4 text-xs text-neutral-500 dark:text-neutral-400 whitespace-nowrap">
                                {order.created_at ? new Date(order.created_at).toLocaleDateString() : 'N/A'}
                              </td>

                              <td className="p-4 text-right whitespace-nowrap">
                                <div className="flex items-center justify-end gap-1.5">
                                  <button type="button" onClick={() => handleDuplicateOrder(order)} className="w-9 h-9 flex items-center justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors" title="Duplicate Order">
                                    <Copy className="w-[18px] h-[18px]" />
                                  </button>
                                  <button type="button" onClick={() => handleEditOrder(order)} className="w-9 h-9 flex items-center justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors" title="Edit Order">
                                    <PencilLine className="w-[18px] h-[18px]" />
                                  </button>
                                  <button type="button" onClick={() => handleDeleteOrder(order.id)} className="w-9 h-9 flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-950/50 rounded-lg text-red-500 transition-colors" title="Delete Order">
                                    <Trash2 className="w-[18px] h-[18px]" />
                                  </button>
                                </div>
                              </td>
                            ...
                            {isExpanded && (
                              <tr className="bg-neutral-50 dark:bg-neutral-950/60">
                                <td colSpan={7} className="p-4 sm:p-5 border-b border-neutral-200 dark:border-neutral-800">
                                  <div className="pl-0 sm:pl-14 space-y-5">
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                      <div>
                                        <h4 className="text-sm font-bold text-neutral-900 dark:text-white">Order Details</h4>
                                        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                          {items.length} product{items.length === 1 ? '' : 's'} · Order #{order.id.slice(0, 8)}
                                        </p>
                                      </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                      <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-4">
                                        <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400 dark:text-neutral-500 mb-3">Customer</div>
                                        <div className="space-y-2 text-sm">
                                          <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Name</div><div className="font-semibold text-neutral-900 dark:text-white">{order.full_name || 'Guest'}</div></div>
                                          <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Email</div><div className="font-medium text-neutral-900 dark:text-white break-all">{order.email || 'N/A'}</div></div>
                                          <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Phone</div><div className="font-medium text-neutral-900 dark:text-white">{order.phone || 'N/A'}</div></div>
                                        </div>
                                      </div>

                                      <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-4">
                                        <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400 dark:text-neutral-500 mb-3">Shipping</div>
                                        <div className="space-y-2 text-sm">
                                          <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Address</div><div className="font-medium text-neutral-900 dark:text-white whitespace-pre-wrap break-words">{order.shipping_address || 'N/A'}</div></div>
                                          <div className="grid grid-cols-2 gap-3">
                                            <div><div className="text-xs text-neutral-500 dark:text-neutral-400">City</div><div className="font-medium text-neutral-900 dark:text-white">{order.city || 'N/A'}</div></div>
                                            <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Postal</div><div className="font-medium text-neutral-900 dark:text-white">{order.postal_code || 'N/A'}</div></div>
                                          </div>
                                          <div><div className="text-xs text-neutral-500 dark:text-neutral-400">Country</div><div className="font-medium text-neutral-900 dark:text-white">{order.country || 'N/A'}</div></div>
                                        </div>
                                      </div>

                                      <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-4">
                                        <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400 dark:text-neutral-500 mb-3">Order Notes</div>
                                        <div className="text-sm text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap break-words min-h-[60px]">
                                          {order.notes?.trim() || 'No order notes.'}
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div>
                                      <div className="flex items-center justify-between gap-3 mb-3">
                                        <div>
                                          <div className="text-[10px] font-semibold uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Ordered Items</div>
                                        ...
                                        {items.length > 0 && (
                                          <div className="text-xs text-neutral-500 dark:text-neutral-400">
                                            {items.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0)} total qty
                                          </div>
                                        )}
                                      </div>
                                      ...
                                      {items.length === 0 ? (
                                        <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-5">
                                          <p className="text-sm text-neutral-500 dark:text-neutral-400">No line items recorded for this order.</p>
                                        </div>
                                      ) : (
                                        <div className="space-y-3">
                                          {items.map((item) => {
                                            ...
                                            const itemColor = item.color || item.selected_color;
                                            const itemCategories = Array.isArray(item.products?.category) ? item.products.category.filter((value) => typeof value === 'string' && value.trim()) : typeof item.products?.category === 'string' && item.products.category.trim() ? [item.products.category] : [];

                                            return (
                                              <div key={item.id} className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-3 sm:p-4">
                                                <div className="flex flex-col sm:flex-row gap-4">
                                                  <div className="shrink-0">
                                                    {img ? (
                                                      <img src={img} alt={item.product_name || 'Ordered item'} className="w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-950" />
                                                    ) : (
                                                      <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center border border-neutral-200 dark:border-neutral-800">
                                                  ...
                                                  <div className="min-w-0 flex-1">
                                                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                                                      <div className="min-w-0">
                                                        <div className="text-base font-bold text-neutral-900 dark:text-white break-words min-w-0">{item.product_name || 'Product'}</div>
                                                        {itemCategories.length > 0 && (
                                                          <div className="flex flex-wrap gap-1.5 mt-1.5">
                                                            {itemCategories.map((itemCategory, categoryIndex) => (
                                                              <span
                                                                key={`${item.id}-item-category-${categoryIndex}`}
                                                                className="inline-flex items-center whitespace-nowrap px-2 py-0.5 rounded-full text-[11px] font-semibold bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300"
                                                              >
                                                                {itemCategory}
                                                              </span>
                                                            ))}
                                                      ...
                                                      <div className="shrink-0 sm:text-right">
                                                        <div className="text-[10px] uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Subtotal</div>
                                                        <div className="text-lg font-mono font-bold text-neutral-900 dark:text-white">
                                                          {order.currency || 'USD'} {(subtotal / 100).toFixed(2)}
                                                        </div>
                                                      </div>
                                                    </div>

                                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-3 border-t border-neutral-100 dark:border-neutral-800">
                                                      <div><div className="text-[10px] uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Quantity</div><div className="mt-1 text-sm font-bold text-neutral-900 dark:text-white">{quantity}</div></div>
                                                      <div><div className="text-[10px] uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Unit Price</div><div className="mt-1 text-sm font-mono font-semibold text-neutral-900 dark:text-white">{order.currency || 'USD'} {(price / 100).toFixed(2)}</div></div>
                                                      <div><div className="text-[10px] uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Color</div><div className="mt-1 text-sm font-semibold capitalize text-neutral-900 dark:text-white">{itemColor || 'N/A'}</div></div>
                                                      <div><div className="text-[10px] uppercase tracking-wider text-neutral-400 dark:text-neutral-500">Product ID</div><div className="mt-1 text-xs font-mono truncate text-neutral-900 dark:text-white">{item.product_id || 'N/A'}</div></div>
                                                    </div>
                                                  </div>
                                                </div>
                                              </div>
                                    ...
                                    <div className="flex justify-end">
                                      <div className="w-full sm:w-80 space-y-2">
                                        <div className="flex justify-between gap-4 text-sm text-neutral-500 dark:text-neutral-400">
                                          <span>Discount</span>
                                          <span className="font-mono">-{order.currency || 'USD'} {((order.discount_amount ?? 0) / 100).toFixed(2)}</span>
                                        </div>
                                        {order.coupon_code && (
                                          <div className="flex justify-between gap-4 text-xs text-neutral-500 dark:text-neutral-400">
                                            <span>Coupon</span>
                                            <span className="font-medium text-neutral-900 dark:text-white">{order.coupon_code}</span>
                                          </div>
                                        )}
                                        <div className="flex justify-between items-center gap-4 pt-3 border-t border-neutral-200 dark:border-neutral-800">
                                          <span className="font-semibold text-neutral-900 dark:text-white">Order Total</span>
                                          <span className="text-lg font-mono font-bold text-neutral-900 dark:text-white">
                                            {order.currency || 'USD'} {((order.total ?? order.amount ?? 0) / 100).toFixed(2)}
                                          </span>
                                        </div>
                                      </div>
                                    </div>
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[900px] text-left text-sm">
                    <thead className="bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400">
                      <tr>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[190px]" onClick={() => handleMessageSort('name')}>
                          <div className="flex items-center gap-1">Sender <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[180px]" onClick={() => handleMessageSort('subject')}>
                          <div className="flex items-center gap-1">Subject <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium whitespace-nowrap min-w-[300px]">Message</th>
                        <th className="p-4 font-medium cursor-pointer whitespace-nowrap min-w-[140px]" onClick={() => handleMessageSort('created_at')}>
                          <div className="flex items-center gap-2 whitespace-nowrap">Date <ArrowUpDown className="w-4 h-4 shrink-0" /></div>
                        </th>
                        <th className="p-4 font-medium text-right whitespace-nowrap min-w-[140px]">Actions</th>
                      </tr>
                    ...
                    <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                      {sortedMessages.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="p-10 text-center text-sm text-neutral-500 dark:text-neutral-400">
                            {messageSearch.trim() ? 'No messages match your search.' : 'No messages found.'}
                          </td>
                        </tr>
                        ...
                        <tr key={msg.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/50 transition-colors">
                          <td className="p-4 min-w-[190px]">
                            <div className="font-semibold text-neutral-900 dark:text-white whitespace-nowrap">{msg.name}</div>
                            <div className="text-xs text-neutral-500 dark:text-neutral-400">{msg.email}</div>
                          </td>
                          <td className="p-4 font-medium text-neutral-900 dark:text-white min-w-[180px]">
                            <div className="whitespace-nowrap">{msg.subject || 'No Subject'}</div>
                          </td>
                          <td className="p-4 text-xs min-w-[300px] max-w-[360px] truncate text-neutral-600 dark:text-neutral-300">
                            {msg.message}
                          </td>
                          <td className="p-4 text-xs text-neutral-500 dark:text-neutral-400 whitespace-nowrap">
