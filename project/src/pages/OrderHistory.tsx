...
const statusConfig: Record<string, { label: string; icon: typeof Clock; color: string }> = {
  ...
  paid: { label: 'Paid', icon: CheckCircle, color: 'text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 dark:bg-indigo-400/10 border-indigo-500/30 dark:border-indigo-400/30' },
...
export default function OrderHistory() {
  ...
  const handleDownloadInstruction = async (orderId: string) => {
    ...
    try {
      ...
      if (!accessToken) {
        alert('Please sign in again before downloading your instructions.');
        return;
      }
